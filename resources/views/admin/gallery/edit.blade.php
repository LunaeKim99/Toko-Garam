...
<div class="flex items-center justify-between mb-8">
    <h1 class="text-2xl font-bold text-[var(--text)]">Edit Galeri</h1>
    <a href="{{ route('admin.galeri.index') }}" class="text-sm text-[var(--text-secondary)] hover:text-[var(--text)]">← Kembali</a>
</div>

<form action="{{ route('admin.galeri.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="bg-[var(--surface)] p-6 lg:p-8 rounded-xl shadow-sm border border-[var(--border)] max-w-xl">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-5">
        ...
        <div>
            <label class="block text-sm font-medium text-[var(--text)] mb-1.5">Gambar (opsional, kosongkan jika tidak diubah)</label>
            @if($gallery->gambar)
